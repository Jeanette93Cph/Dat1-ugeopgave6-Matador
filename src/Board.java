...
public class Board {
    ...
    public static int numberOfChanceCard = 0;
    ...
    * */
    ...
    private void createChanceCards(String[] data)
    {
        ...
        {
            String s = data[i];
            // nedenstående hjælper med at det ikke bliver 1 string, men flere opdelt med ','
            String[] values = s.split(",");
            String name = values[0];
            int cost = Integer.parseInt(values[1]);
            int payout = Integer.parseInt(values[2]);

            ChanceCard chancecards1 = new ChanceCard(name, cost, payout);
            chanceCards[i] = chancecards1;
        }
    ...
    {
        ChanceCard card = chanceCards[numberOfChanceCard];
        numberOfChanceCard ++;
        if(numberOfChanceCard == chanceCards.length)
        {
            numberOfChanceCard = 0;
        }
        return card;
    }
